...
void X86_64ABIInfo::classify(QualType Ty, uint64_t OffsetBase,
  ...
  if (Ty->hasPointerRepresentation()) {
    ...
    // the Lo part to be Integer.  _MM_ptr and _MM_array_ptr also needs to
    // set the Hi part for its key and offset, and _MM_large_ptr has 192-bit
    // and thus it should be passed by memory.
    // Note we only implemented for X86-64 ABI for now.
    if (Ty->isCheckedPointerMMType() || Ty->isCheckedPointerMMArrayType()) {
      Hi = Integer;
...
ABIArgInfo X86_64ABIInfo::getIndirectReturnResult(QualType Ty) const {
  ...
  // Checked C: _MM_large_ptr is returned by memory.
  if (!isAggregateTypeForABI(Ty) && !Ty->isCheckedPointerMMLargeType()) {
    // Treat an enum type as its underlying type.
    if (const EnumType *EnumTy = Ty->getAs<EnumType>())
      Ty = EnumTy->getDecl()->getIntegerType();
