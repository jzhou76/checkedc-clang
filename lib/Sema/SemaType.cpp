...
QualType Sema::BuildPointerType(QualType T, CheckedPointerKind kind,
  ...
  // In Checked C, _TS_ptr is only allowed to point to struct types.
  // One special case is when a _TS_ptr is in a generic function, the
  // type of its pointee is allowed to be a TypeVariableType.
  if (kind == CheckedPointerKind::TSPtr &&
      (!T->isStructureType() && !T.getTypePtr()->getAs<TypeVariableType>())) {
    Diag(Loc, diag::err_illegal_decl_ts_ptr_to_non_struct)
      << getPrintableNameForEntity(Entity) << T;
    return QualType();
  }
